...
_NATURAL_FR_DATE_PATTERN = re.compile(
    r"\b(\d{1,2})\s+(janvier|février|fevrier|mars|avril|mai|juin|juillet|août|aout|septembre|octobre|novembre|décembre|decembre)\s+(\d{2,4})\b",
    re.IGNORECASE,
)
...
def _extract_birth_date(message: str) -> tuple[str | None, list[str]]:
    ...
    natural_match = _NATURAL_FR_DATE_PATTERN.search(message)
    if natural_match is not None:
        day = int(natural_match.group(1))
        month = _FRENCH_MONTHS.get(natural_match.group(2).lower())
        ...
        if month is not None:
            ...
            if iso is not None:
                return iso, ["natural_french_birth_date"]
...
def parse_profile_collect_message(message: str) -> dict[str, ParsedValue]:
    ...
    if birth_date is not None:
        ...
        name_source = _NATURAL_FR_DATE_PATTERN.sub(" ", name_source)
