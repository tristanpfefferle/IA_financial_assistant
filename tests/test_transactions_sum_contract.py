...
def test_sum_all_returns_decimal_and_count() -> None:
    ...
    result = agent_loop.tool_router.call("finance_transactions_sum", {}, profile_id=PROFILE_ID)
...
def test_sum_debit_only_is_negative_or_zero_and_count_matches() -> None:
    ...
    debit_result = agent_loop.tool_router.call(
        "finance_transactions_sum", {"direction": "DEBIT_ONLY"}, profile_id=PROFILE_ID
    )
    base_result = agent_loop.tool_router.call("finance_transactions_search", {}, profile_id=PROFILE_ID)
...
def test_sum_with_merchant_filter() -> None:
    ...
    result = agent_loop.tool_router.call(
        "finance_transactions_sum", {"merchant": "coffee"}, profile_id=PROFILE_ID
    )
...
def test_sum_invalid_direction_validation_error() -> None:
    ...
    result = agent_loop.tool_router.call(
        "finance_transactions_sum", {"direction": "WRONG"}, profile_id=PROFILE_ID
    )
